...
class LottoGeneratorLayout(BoxLayout):
    ...
    def _check_updates_async(self, dt):
        """비동기로 업데이트 확인"""
        try:
            needs_update, message = self.local_db.check_for_updates()
            
            if needs_update:
                self.ids.db_status_label.text = f"업데이트 가능: {message}"
                # 업데이트 버튼을 UI에 추가하거나 자동 업데이트 실행
                self.show_update_notification(message)
            else:
    ...
    def show_update_notification(self, message):
        ...
        content.add_widget(Label(text=f"새로운 로또 데이터가 있습니다.\n{message}", text_size=(300, None), halign='center'))
        
        btn_layout = BoxLayout(spacing='10dp', size_hint_y=None, height='48dp')
        
        update_btn = Button(text="업데이트", size_hint_x=0.5)
        update_btn.bind(on_press=lambda x: self.perform_update())
        
        later_btn = Button(text="나중에", size_hint_x=0.5)
        later_btn.bind(on_press=lambda x: self.update_popup.dismiss())
